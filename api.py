...
Usage:
    # Start server
    uvicorn api:app --host 0.0.0.0 --port 8704

    # Train
    curl -X POST http://localhost:8704/api/train \
      -F "files=@template.jpg" \
      -F "image_res=512"

    # Detect
    curl -X POST http://localhost:8704/api/detect \
      -F "file=@test.jpg" \
      -F "viz_mode=overlay"
"""
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("PORT", 8704))
