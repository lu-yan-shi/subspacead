...
from transformers.models.metaclip_2.configuration_metaclip_2 import (
    MetaClip2Config, 
    MetaClip2TextConfig, 
        ...
        if self.config._attn_implementation != "eager":
            if "output_attentions" in kwargs and kwargs["output_attentions"]:
                if self.config._attn_implementation == "sdpa":
                    logger.warning_once(
                        "Outputting attentions is not supported with SDPA attention implementation. "
                        "Falling back to eager attention."
                    )
                else:
                    logger.warning_once(
                        f"Outputting attentions is not supported with {self.config._attn_implementation} attention implementation. "
                        "Falling back to eager attention."
                    )
            else:
